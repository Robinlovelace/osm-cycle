...
library(knitr)
kable(head(NL[ order(NL$Abs.Growth, decreasing=T), c("NAME", "Abs.Growth") ], 15))
```
...
ggmap(get_map(bbox(msoa.leeds))) + 
  geom_polygon(data = msoa.leeds.f, aes(long, lat, group=group, fill=Abs.Growth), alpha=0.8) +  
...
```{r, warning=FALSE, message=FALSE}
ggmap(get_map(bbox(msoa.leeds))) + 
  geom_polygon(data = msoa.leeds.f, aes(long, lat, group=group, fill=pCycle * 100), alpha=0.8) +  
  scale_fill_gradient(low="red", high="green", name = "Cycle commuters\n2011 (%)")
```
