...
# la01.pnts@data <- rbind(la01.pnts@data, c("Cornwall,Isles of Scilly", colSums(la01.pnts@data[corn.sel, 2:16])))
newCornw <- SpatialPointsDataFrame(coordinates(la11[ grepl("Cornw", la11$NAME), ]), match.ID= F,
                                         data = data.frame(t(c(ZONE_LABEL =1000, colSums(la01.pnts@data[corn.sel, 2:16])))) )
...
# total zones merged
length(corn.sel) + length(Wilt) + length(Northumb) + length(Durh) + 3 + 3 + 5 + length(Chest)  
```

## Now merge the results

```{r}
la11merge <- merge(la01.pnts@data, la11@data, by.x = "ZONE_LABEL", by.y = "NAME", all.x=F)
lam <- la11merge ; rm(la11merge)

# allocate cdm funding
lam$cdm <- "No"
cdt05 <- grepl("Ayl|Bright|Darl|Derby$|Exeter|Lancaster$", lam$ZONE_LABEL)
lam[cdt05, ] 
lam$cdm[cdt05] <- "2005" 
cdt09 <- grepl("Blackpo|Bristol|Cambridge$|Chester$|Colches|Bedf|Shrop|Southend|Sefton|Stoke|Woking$|York$", lam$ZONE_LABEL)
lam[cdt09, ] 
lam$cdm[cdt09] <- "2009" 
...
hist(lam$Growth)
qplot(data=lam, x = Growth, geom="histogram", fill = cdm)
