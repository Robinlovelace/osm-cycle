...
while leaving the root causes un-addressed (e.g. Sheehan et al. 2009).
Various theories have been developed to explain our collective failure (Berners-Lee and Clarke, 2012).
...
transport to work. Yet the subject matter was influenced by and contributes to
discussion of humanity's short term sustainability and
longer term survival. Many of the benefits of cycling uptake now may only
be realised long in the future, yet many papers evaluating bicycle policies
focus solely on 'the here and now'. This bias towards the present is present
in many areas, not least in economics, and deserves wider discussion
in the context transport planning. For now, however, let us press on with the
specific matter of the paper: cycling to work.
...
array of issues in one simple step. Commuting is an important reason for personal
...
on the benefits of active travel more broadly.
There is less research, however, about which policies are most
likely to boost cycling in different places at the sub national level. There have been international
comparative studies on why cycling flourishes in some countries whilst barely accounting
for 1% of trips in others (citep{Pucher}. Research seeking to explain variation in
bicycle use between smaller administrative zones is described in section x.
...
growth in cycling at the national level? A central motivation of
this paper is to shed light on potential answers to this question.
...
The expectations were, certeris parabis (all things being being equal), that:
...
- *Invst*, a crude proxy of investment in cycling in each zone, defined as a binary variable:
...
These are described as and when they appear in the results.

# Method

The proportion of people cycling to cycling to work
...
$$PCycle = \sum_{a=1}^n \frac{Commute_a}{\sum_{a=1}^n Commute} \frac{Cycle_a}{Commute_a} $$
...
the total number of commuters ($PCycle = Cycle / Commute$). However, equation x
...
The regional pattern represented by the colours in fig. x
...
and 14% of the variability in $Q pCycle$ across all zones (Table x).
This is a statistically
significant by not particularly powerful result. In addition, the most
statistically significant variable, $Bcrash$, had the opposite sign as
expected, indicating a cycling may have become proportionally riskier
in places where cycling has grown the most. Indeed,
...
in areas where cycling grew.

$Avinc$ and $Invst$ had a statistically significant (p > 5%)
impact on the model in the direction expected, with average income
in 2004/5 having a strong positive correlation with $\Delta pCycle$ 
...
As can be seen in figure xx, LAs within London had anomalously high growth
...
The use of linear regression as a tool to explain changes
over space
